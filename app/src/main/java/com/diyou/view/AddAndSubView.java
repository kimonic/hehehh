...
public class AddAndSubView extends LinearLayout
{
	...
	private void initialise()
	{
		...
		addButton.setText("+");subButton.setText("-");
		addButton.setTag("+");
		subButton.setTag("-");
		//设置输入类型为数字
		editText.setInputType(android.text.InputType.TYPE_CLASS_NUMBER);
		editText.setText(String.valueOf(num));
		editText.setFilters(new InputFilter[]{new InputFilter.LengthFilter(MAX_LENGTH)});//设置最大长度
	}
	...
	/**
	 * 以资源Resource形式 设置按钮背景图
	 * @param editTextBgResource editText背景图
	 */
	public void setEditTextBgResource(int editTextBgResource)
	{
		editText.setBackgroundResource(editTextBgResource);
		editText.setGravity(Gravity.CENTER);
	}
		...
		public void afterTextChanged(Editable s)
		{
			...
			if(numString == null || numString.equals(""))
			{
				...
				if(numInt>maxNum){
					...
					Toast.makeText(context, "请输入一个小于" + maxNum + "的数字",
							Toast.LENGTH_SHORT).show();
				}
				else if (numInt == minNum)
				{
					num=minNum;
